...
def main():
    ...
    screen = p.display.set_mode(
        (BOARD_WIDTH+MOVE_LOG_PANEL_WIDTH+100, BOARD_HEIGHT+15))
    ...
    while running:
        ...
        for e in p.event.get():
            ...
            # mouse handler
                ...
                if not gameOver:
                    ...
                    # the user clciked the same square twice or user clicked moves log
        ...
        drawGameState(screen, gs, validMoves, sqSelected,
                      moveLogFont, playerOne, playerTwo, Choice)
...
def drawBoard(screen, choice):
    ...
    for r in range(DIMENSION):
        for c in range(DIMENSION):
            ...
            p.draw.rect(screen, colors[((c + r) % 2)],
            ...
            if r == 7:
                ...
                text = chr(ord(text) + 1)
                textLocation = p.Rect(
                    c*SQ_SIZE, r*SQ_SIZE, SQ_SIZE, SQ_SIZE).move(1, 50)
                screen.blit(textObject, textLocation)
            if c == 7:
                textObject = font.render(
                    str(num), True, colors[((c + r+1) % 2)])
                num = num-1
                textLocation = p.Rect(
                    c*SQ_SIZE, r*SQ_SIZE, SQ_SIZE, SQ_SIZE).move(51, 1)
                screen.blit(textObject, textLocation)
    ...
    # drawBoard(screen,choice)
        ...
        if move.pieceCaptured != '--':
            if move.isEnpassantMove:
                ...
                endSquare = p.Rect(move.endCol*SQ_SIZE,
                                   move.enPassantRow*SQ_SIZE, SQ_SIZE, SQ_SIZE)
...
def drawStartGameText(screen, font):
    boundRect = p.Rect(BOARD_WIDTH+MOVE_LOG_PANEL_WIDTH,
                       0, 100, MOVE_LOG_PANEL_HEIGHT)
